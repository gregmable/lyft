...
FARE_URL = "https://www.lyft.com/rider/fare-estimate"
...
class LyftClient:
    ...
    def _debug_screenshot_path(self, attempt: int) -> Path:
        ...
        return debug_dir / f"lyft_attempt_{attempt}_{timestamp}.png"
    ...
    def get_cost_estimate(self) -> dict[str, Any]:
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                with sync_playwright() as playwright:
                    ...
                    if not from_ok:
                        raise RuntimeError("Could not locate Lyft pickup input")
                    ...
                    if not to_ok:
                        raise RuntimeError("Could not locate Lyft destination input")
                    ...
                    if not extracted:
                        raise RuntimeError("Unable to parse Lyft price from fare estimate page")
                ...
                last_error = RuntimeError("Lyft web quote timed out while loading fare estimate")
                if page is not None:
        ...
        if last_error is None:
            last_error = RuntimeError("Lyft web quote failed for unknown reason")

        try:
            fallback = estimate_fallback_fare(self.settings, provider="lyft")
            ...
            raise RuntimeError(
                f"Lyft web quote failed after {retries} attempt(s): {last_error}; fallback failed: {fallback_exc}"
            )
